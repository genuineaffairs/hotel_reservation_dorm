...
class HotelBed(models.Model):
	...
	def check_availability(self, check_in, check_out):
		self.env.cr.execute("SELECT * FROM hotel_room_reservation_line WHERE (check_in,check_out) OVERLAPS ( timestamp %s, timestamp %s ) AND bed_id=%s AND state='unassigned'", (check_in, check_out, self.id))
